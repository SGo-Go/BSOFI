...
\begin{abstract}
  ...
  \begin{keywords}  
    ...
    GPU accelerated linear algebra %; \Cuda multi-GPU programming
  \end{keywords}
\end{abstract}
...
physical observables~\cite{Bai09,Tomas12}. %Loh92,
...
the conventional Gaussian elimination with row partial pivoting %(GEPP)
...
\begin{figure}[t]%[thb]
  ...
  \begin{subfigure}[t]{0.45\linewidth}%  \subfigure[Row Version]
    \begin{algorithm}[H]
      ...
      \For{$i \in \{p-3, p-4,..., 1 \}$ }{ 
        $X_{i,i+2:p} \gets X_{i,i+2:p} + X_{i,i+1}\cdot X_{i+1,i+2:p} $ \;  
        $X_{i,i+1} \gets X_{i,i+1} \cdot X_{i+1,i+1}$\;}
    \end{algorithm}    
...
the total flops complexity %$f(n,p)$ 
...
% Here, $CR$ can be either $CPU$ or $GPU$ respectively.
...
\begin{align*}
  ...
  , \,
  c''_k = {%\kappa_Q 
    \frac{T^{GPU}_{\texttt{SET(n,n)}}}{%\{T_{\texttt{DGEMM(2*n,n,n)}}^{GPU},T^{GPU}_{\texttt{GET(n,n)}}\}
...
against \CuBlas, \Magma, and %the \Blas and \Lapack provided by 
Intel's MKL library.
...
\textit%%\paragraph
{Performance tuning.} %Performance of building blocks and validation of load balancing approach
...
in-place inversion of $p$-cyclic matrices. % The key trait of our 
