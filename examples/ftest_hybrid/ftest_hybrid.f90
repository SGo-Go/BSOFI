...
! ------------------------------------------------------------------
!  Description:
!   Example of use with Fortran (CPU+GPU codes based on LAPACK 
...
subroutine InvertA_BSOFI(n, nb, A, s)
  ...
  integer  :: hybridXbsofiLWorkH
  external    hybridXbsofiLWorkH
  ...
  call hybridXinitH(handler, n, nb, nnb)

  lw = hybridXbsofiLWorkH(handler, n, nb, nnb)
  allocate(W1(lw))
...
program TestInverse
  ...
  integer, parameter    :: n  = 1024!128!512!216
